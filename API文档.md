...
- **请求参数**:
  ...
  - `parentId` (Long, required): 父分类ID（一级分类填0）
  - `sortOrder` (Integer, required): 排序号
- **请求示例**:
```json
{
  "name": "热菜",
...
- **请求头**: `Content-Type: application/json`
- **请求参数**:
  -  `id` (Long, required): 分类 ID
  - `name` (String, optional): 分类名称
  - `parentId` (Long, optional): 父分类 ID
  - `sortOrder` (Integer, optional): 排序号
- **请求示例**:
```json
{
  "id": 2,
